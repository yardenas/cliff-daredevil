...
class FrictionZoneListener(b2.b2ContactListener):
    ...
    def _contact(self, contact, begin):
        if contact.fixtureA == self.env.friction_zone or contact.fixtureB == self.env.friction_zone:
            if begin:
...
class CliffDarvedevil(gym.Env):
    ...
    def __init__(self, friction_profile=0.1):
        ...
        self.friction = friction_profile if callable(
            friction_profile) else lambda _: friction_profile
    ...
    def _build_road(self):
        self.ground = self.world.CreateStaticBody(fixtures=[b2.b2FixtureDef(
            shape=b2.b2EdgeShape(vertices=[(self.min_position, ROAD_HIGHT),
                                           (self.goal_zone[1] + self.cliff_edge, ROAD_HIGHT)]),
            friction=0.99),
            b2.b2FixtureDef(shape=b2.b2EdgeShape(vertices=[(self.friction_start, ROAD_HIGHT),
    ...
    def step(self, action):
        ...
        if self.friction_zone.touch:
            progress = np.clip((x - self.friction_start) / (
                    self.goal_zone[1] - self.friction_start), 0.0, 1.0)
    ...
    def render(self, mode='human'):
        ...
        if self.viewer is None:
            ...
            sky = rendering.make_polygon([(self.min_position, 0.0), (self.min_position, 40.0),
                                          (self.max_position, 40.0), (self.max_position, 0.0)])
            ...
            ground = rendering.make_polygon([(self.min_position, 0.0),
                                             (self.min_position, ROAD_HIGHT)]
                                            + [*zip(xs_ground, ys_ground)])
            ground.set_color(237 / 255, 201 / 255, 175 / 255)
            oil = rendering.make_polyline([(self.friction_start, ROAD_HIGHT - 0.1),
            ...
            sea = rendering.make_polygon([*zip(xs_sea, ys_sea)] + [(xs_sea[-1], 0.0),
                                                                   (xs_sea[0], 0.0)])
            ...
            frontwheel.add_attr(
                rendering.Transform(translation=(car_width / 4, 0)))
            ...
            def make_flag(position):
                ...
                flag = rendering.FilledPolygon(
                    [(flagx, flagy2), (flagx, flagy2 - 1.0), (flagx + 2.5, flagy2 - 0.5)])
